...
describe('BulbsElements Store', function () {
  ...
  it('merges actions from each field', function () {
  ...
  it('curries the given action with the state', function () {
  ...
  it('shallow clones the entire state object', function () {
    let originalState = subject.state;
    subject.actions.addTen();
    assert(originalState !== subject.state);
  });

  it('clones the field value associated with the action', function () {
    let originalBar = subject.state.bar;
    subject.actions.updateBaz('new baz');
    assert(originalBar !== subject.state.bar);
  });

  it('doesn\'t clone fields not associated with the actions', function () {
    let originalBar = subject.state.bar;
    subject.actions.addTen();
    assert(originalBar === subject.state.bar);
  });

  it('doesn\'t allow action name collisions', () => {
    schema = {
      field1: {
        actions: {
          handleSomeEvent() {},
    ...
    expect(function() {
      new Store({ schema }); // eslint-disable-line
    }).to.throw('Store: duplicate action named "handleSomeEvent"');
  });
});
