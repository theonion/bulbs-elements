...
function maybeLoadComponents () {
  ...
  animationFrameRequested = requestAnimationFrame(() => {
...
export default function loadOnDemand (Component) {
  if (Component.prototype instanceof HTMLElement) {
    return class Wrapped extends Component {
      ...
      isInLoadState () {
        return this.state.load || this.hasAttribute('disable-lazy-loading');
      }

      createdCallback () {
        this.state || (this.state = { });
        if (this.isInLoadState()) {
          super.createdCallback && super.createdCallback();
        }
      }

      attachedCallback () {
        if (this.isInLoadState()) {
          super.attachedCallback && super.attachedCallback();
        }
        else {
          enqueue(this);
        }
      ...
      detachedCallback () {
        if (this.isInLoadState()) {
          super.detachedCallback && super.detachedCallback();
        }
        else {
          dequeue(this);
        }
      ...
      attributeChangedCallback (attrName, newValue, oldValue) {
        if (this.isInLoadState()) {
          super.attributeChangedCallback && super.attributeChangedCallback(attrName, newValue, oldValue);
        }
      }
    };
  }
