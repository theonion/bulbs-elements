...
describe('InViewMonitor', () => {
  ...
  describe('progress events', () => {
    ...
    it('emits event when element is nearing viewport from bottom', () => {
      let windowHeight = window.innerHeight;
      el.style.top = windowHeight + 500 + 'px';
      InViewMonitor.add(el);
      el.addEventListener('approachingviewport', spy);
      el.style.top = windowHeight + 200 + 'px';
      InViewMonitor.checkElement(el);
      expect(spy).to.have.been.called.once;
    });
  });
  ...
  describe('isElementInViewport', () => {
    it('zero height element at top of viewport is in view', () => {
      el.style.height = '0px';
      el.style.position = 'fixed';
      el.style.top = '0px';

      expect(InViewMonitor.isElementInViewport(el)).to.be.true;
    });
  });
