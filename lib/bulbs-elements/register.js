    ...
    */
    attachedCallback () {
      if (!this.reactElement) {
        let renderableReactElement = React.createElement(ReactComponent, this.attributesHash);
    ...
    get onionEditorContent () {
      return `<${tagName}${this.editorAttributes}></${tagName}>`;
    }

    get editorAttributes () {
      let attributes = this.attributesHash;
      let attributeString = Object.keys(this.attributesHash).map((key) => {
...
export function registerForBulbsCMS (tagName, ElementClass) {
